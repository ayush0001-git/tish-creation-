...
const MAX_ITEMS = 100;
/* ── SHIPPING RULES (v2.2 — owner's new policy) ────────────────────────────
     • FREE shipping when the goods subtotal is ₹1,000 or more — for BOTH
       payment modes (UPI/prepaid AND Cash on Delivery).
     • Small (1–2 item) orders below ₹1,000 are now ALLOWED — the old ₹1000
       minimum-order gate is gone. The customer instead pays the REAL courier
       charge for their pincode, quoted live from the Delhivery rate API
       (origin = WAREHOUSE_PINCODE, weight estimated from item count).
     • If Delhivery isn't configured or the API errors, checkout never breaks:
       we fall back to a flat ₹60 (prepaid) / ₹80 (COD).
   The storefront (index.html) mirrors this math exactly, and quotes are
   cached for 15 minutes so the price the customer SAW at checkout is the
   same price the order charges — which is also what keeps the payment
   webhook's amount cross-check passing for legitimate payments. */
const RETAIL_FREE_SHIP_THRESHOLD = 1000;  // ₹ — free shipping at/above this subtotal
const FLAT_SHIP_PREPAID = 60;             // ₹ — fallback when the live rate is unavailable
const FLAT_SHIP_COD = 80;                 // ₹ — fallback (COD costs the courier more)
const SHIP_QUOTE_TTL_MS = 15 * 60 * 1000; // quote cache TTL: keeps "quoted" == "charged"
const shipQuoteCache = new Map();         // `${pin}|${grams}|${mode}` → { amount, at }

// Deterministic parcel-weight estimate SHARED with the storefront:
// 150 g per item, billed minimum 500 g, capped at 50 kg. index.html uses the
// identical formula (0.15 kg × qty, min 0.5 kg) so client quote == server charge.
function estimateWeightGrams(itemCount) {
  const n = Math.max(1, parseInt(itemCount, 10) || 1);
  return Math.min(50000, Math.max(500, n * 150));
}

// Live Delhivery forward-shipping rate (warehouse → destination pincode),
// cached for SHIP_QUOTE_TTL_MS. Returns a rounded ₹ amount, or null on any
// failure — the caller decides the flat fallback.
async function delhiveryRateQuote(pincode, grams, mode) {
  if (!DELHIVERY_TOKEN || !axios || !isValidPincode(pincode)) return null;
  const key = `${pincode}|${grams}|${mode}`;
  const hit = shipQuoteCache.get(key);
  if (hit && Date.now() - hit.at < SHIP_QUOTE_TTL_MS) return hit.amount;
  try {
    const r = await axios.get('https://track.delhivery.com/api/kinko/v1/invoice/charges/.json', {
      params: { md: mode === 'cod' ? 'S' : 'E', cgm: grams, o_pin: WAREHOUSE_PINCODE, d_pin: pincode, ss: 'Delivered' },
    ...
    if (amount > 0) { shipQuoteCache.set(key, { amount, at: Date.now() }); return amount; }
    return null;
  } catch (e) { console.error('delhivery rate:', e?.message); return null; }
}

// Live Delhivery package tracking by AWB / waybill number, for the customer
// "Track order" screen. Cached 5 minutes per waybill; failures return null so
// the DB status still renders fine without the live courier feed.
const trackCache = new Map(); // waybill → { data, at }
async function delhiveryLiveTrack(waybill) {
  if (!DELHIVERY_TOKEN || !axios || !waybill) return null;
  const hit = trackCache.get(waybill);
  if (hit && Date.now() - hit.at < 5 * 60 * 1000) return hit.data;
  try {
    const r = await axios.get('https://track.delhivery.com/api/v1/packages/json/', {
      params: { waybill, verbose: 0 },
      headers: { Authorization: `Token ${DELHIVERY_TOKEN}` }, timeout: 8000,
    });
    const sh = r.data?.ShipmentData?.[0]?.Shipment;
    if (!sh) return null;
    const st = sh.Status || {};
    const data = {
      status: st.Status || null,            // e.g. "In Transit", "Dispatched", "Delivered"
      location: st.StatusLocation || null,
      time: st.StatusDateTime || null,
      instructions: st.Instructions || null,
      expected: sh.ExpectedDeliveryDate || sh.PromisedDeliveryDate || null,
      destination: sh.Destination || null,
    };
    trackCache.set(waybill, { data, at: Date.now() });
    return data;
  } catch (e) { console.error('delhivery track:', e?.message); return null; }
}

/* Shipping fee for the AUTHORITATIVE order total. Mirrors the storefront:
     subtotal >= ₹1000              → FREE (prepaid AND COD)
     subtotal <  ₹1000 + Delhivery  → live pincode rate (15-min cached quote)
     subtotal <  ₹1000, no rate     → flat ₹60 prepaid / ₹80 COD            */
async function orderShippingFee(paymentMode, subtotal, pincode, itemCount) {
  const s = Number(subtotal) || 0;
  if (s >= RETAIL_FREE_SHIP_THRESHOLD) return 0;
  const mode = paymentMode === 'cod' ? 'cod' : 'prepaid';
  const live = await delhiveryRateQuote(pincode, estimateWeightGrams(itemCount), mode);
  if (live != null) return live;
  return mode === 'cod' ? FLAT_SHIP_COD : FLAT_SHIP_PREPAID;
}
...
async function computeOrderTotals(items, paymentMode, pincode) {
  ...
  // v2.2: shipping is pincode-aware — free ≥ ₹1000, else the live Delhivery
  // rate for this destination (flat fallback). See orderShippingFee above.
  const shipping = await orderShippingFee(paymentMode, subtotal, pincode, itemCount);
...
app.post('/api/admin/login', authLimiter, async (req, res) => {
  ...
  // When an owner email is configured, require it to match (email + password login).
  // If ADMIN_EMAIL is unset, login stays password-only (backward compatible).
  if (ADMIN_EMAIL) {
    const supplied = String(email || '').trim().toLowerCase();
    if (supplied !== ADMIN_EMAIL.trim().toLowerCase()) {
  ...
  if (!ok) {
    registerAdminFail(ip);
    return res.status(401).json({ error: ADMIN_EMAIL ? 'Incorrect email or password' : 'Incorrect password' });
  }
  clearAdminFails(ip);
  try {
...
app.post('/api/shipping/rate', async (req, res) => {
  // v2.2: uses the same cached delhiveryRateQuote() the order total uses, so
  // the number quoted here is EXACTLY what /api/orders will charge. If the
  // live rate is unavailable we return the flat fallback (never a 502) so
  ...
  const m = mode === 'cod' ? 'cod' : 'prepaid';
  const flat = m === 'cod' ? FLAT_SHIP_COD : FLAT_SHIP_PREPAID;
  const live = await delhiveryRateQuote(String(pincode), Math.round(w * 1000), m);
  res.json({ demo: live == null, amount: live == null ? flat : live, currency: 'INR', freeAbove: RETAIL_FREE_SHIP_THRESHOLD });
});
...
app.post('/api/shipping-price', async (req, res) => {
  // v2.2: same cached quote as the order total — quoted == charged. Flat
  // fallback instead of a 502 so the storefront checkout never dead-ends.
  const b = req.body || {};
  const pincode = String(b.dest_pin || '');
  const mode = b.cod ? 'cod' : 'prepaid';
  const w = Math.max(0.05, Math.min(50, Number(b.weight) || 0.5));
  if (!isValidPincode(pincode)) return res.status(400).json({ error: 'Enter a valid 6-digit pincode' });
  const flat = mode === 'cod' ? FLAT_SHIP_COD : FLAT_SHIP_PREPAID;
  const live = await delhiveryRateQuote(pincode, Math.round(w * 1000), mode);
  const total = live == null ? flat : live;
  res.json({ total, total_charge: total, currency: 'INR', freeAbove: RETAIL_FREE_SHIP_THRESHOLD, demo: live == null });
});
...
app.post('/api/orders', orderLimiter, async (req, res) => {
  ...
  // v2.2: the old ₹1000 minimum-order gate is REMOVED — small (1–2 item)
  // orders are now welcome; below ₹1000 the customer simply pays the live
  // pincode-based Delhivery shipping charge (see orderShippingFee). The
  // constant + check stay in place so the owner can re-enable a minimum
  // later by changing this one number.
  ...
  try {
    ...
    // Server-side minimum-order enforcement (currently 0 = disabled; kept so a
    // future minimum can't be bypassed by calling the API directly).
...
app.get('/api/orders/track', async (req, res) => {
  ...
  try {
    ...
    if (!isOwner && !guestOk && !req.session?.isAdmin) return res.status(403).json({ error: 'Not allowed' });
    // v2.2: when the owner has attached a Delhivery AWB (tracking_number),
    // enrich the response with the LIVE courier scan — current status,
    // location, and expected delivery date — straight from the Delhivery
    // tracking API (5-min cache). Failures are silent; the DB status still
    // renders a full timeline on the storefront.
    const courier = o.tracking_number ? await delhiveryLiveTrack(String(o.tracking_number)) : null;
    res.json({
      order: {
